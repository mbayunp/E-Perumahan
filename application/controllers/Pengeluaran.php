...
class Pengeluaran extends CI_Controller
{
    ...
    function tabel()
    {
        $data['pengeluaran'] = $this->crud_models
            ->view(null, 'tbl_pengeluaran', null)
            ->result();
    ...
    function add()
    {
        ...
        $data = [
            ...
            'harga' => $post['harga'],
            'jumlah' => $post['jumlah'],
            'nominal' => $post['harga'] * $post['jumlah'],
        ...
        if ($type_save == 'add') {
            ...
            $save = $this->crud_models->update('tbl_pengeluaran', $dataNya, [
                'id' => $id,
            ]);
        }
    ...
    function edit($id)
    {
        $data = $this->crud_models
            ->view(null, 'tbl_pengeluaran', ['id' => $id])
            ->row();
    ...
    function cekIuran()
    {
        $nominal = $this->db
            ->query(
                'select sum(nominal_iuran) as nominal from tbl_iuran where status=1'
            )
            ->row()->nominal;
        $pengeluaran = $this->db
            ->query('select sum(nominal) as nominal from tbl_pengeluaran ')
            ->row();
